...
function CorrectionForm() {
  ...
  const [targetRecord, setTargetRecord] = useState<string>(''); // Will hold 'household:{id}' or 'person:{id}'
  ...
  useEffect(() => {
    async function fetchHousehold() {
      ...
      try {
        ...
        // Default to correcting the household itself
  ...
  return (
    <form onSubmit={handleSubmit} className="card p-8 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Input label="Correct City" value={changes.city} onChange={e => handleChange('city', e.target.value)} />
